...
def account_register(request):
    ...
    if request.method == 'POST':
        if userForm.is_valid() and voterForm.is_valid():
            ...
            entered_email = userForm.cleaned_data.get('email')
            allowed_emails = getattr(settings, 'ALLOWED_EMAILS', [])

            if entered_email not in allowed_emails:
                # If the email is not allowed, display an error message
                messages.error(request, "Email not allowed for registration.")
                return render(request, "voting/reg.html", context)

            # Save the user and voter data if validation passes
